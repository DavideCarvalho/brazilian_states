...
describe('state', () => {
  ...
  it('should return only the name of the state if returnEntireJson is set to false', () => {
    expect(api.getCityState({ city: 'Santos', returnEntireJson: false })).to.be.equal('São Paulo');
  });
  it('should return empty array if nothing is found and returnEntireJson is true', () => {
    expect(
      api.getCityState({ city: 'asldjoijsda', returnEntireJson: true }),
    ).to.be.deep.equal({});
  });
  it('should return empty string if nothing is found and returnEntireJson is false', () => {
    expect(
      api.getCityState({ city: 'asldjoijsda', returnEntireJson: false }),
    ).to.be.deep.equal('');
  });
