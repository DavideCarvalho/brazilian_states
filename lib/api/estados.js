...
const normalizedCities = _lodash2.default.map(states, state => {
  const stateCitiesNormalized = _lodash2.default.map(state.cities, city => (0, _removeAccents2.default)(city.replace(/\s|-|_/g, '').toLowerCase()));
...
api.getAllRegions = ({ shouldReturnEntireJson = false }) => {
  ...
  if (shouldReturnEntireJson) {
    return _lodash2.default.map(regions, region => region);
  }
  return _lodash2.default.map(regions, region => region.regionName);
};

api.getStateRegion = ({ state = requiredParam('state') }) => _lodash2.default.find(regions, region => _lodash2.default.find(region.states, regionState => {
  const normalizedStateName = (0, _removeAccents2.default)(state.toLowerCase());
  return (0, _removeAccents2.default)(regionState.state.toLowerCase()) === normalizedStateName;
}));
...
api.getRegion = ({ region = requiredParam('region') }) => {
  ...
  const normalizedRegionName = (0, _removeAccents2.default)(region.replace(/\s|-|_/g, '').toLowerCase());
  return regions[normalizedRegionName];
};
...
api.getStateCities = ({ state = requiredParam('state') }) => {
  ...
  const findState = element => (0, _removeAccents2.default)(element.state.replace(/\s|-|_/g, '').toLowerCase()) === normalizedState || element.abbreviation === normalizedState;
