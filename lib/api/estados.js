...
api.getRegion = ({ region = requiredParam('region') }) => {
  if (Array.isArray(region)) {
    return _lodash2.default.map(region, singleRegion => {
      const normalizedRegionName = (0, _removeAccents2.default)(singleRegion.replace(/-/g, '').toLowerCase());
      return regions[normalizedRegionName];
    });
  }
  const normalizedRegionName = (0, _removeAccents2.default)(region.replace(/-/g, '').toLowerCase());
  return regions[normalizedRegionName];
};
